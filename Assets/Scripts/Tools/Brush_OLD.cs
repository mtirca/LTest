...
namespace Pick.Mode
{
    public class Brush_OLD : MonoBehaviour
    {
        ...
        {
            if (_quitting) return;
            if (artefact != null && artefact.ShaderUpdater != null)
            {
        ...
        private void Update()
        {
            if (_activeLabel == null || !Input.GetMouseButton(0) || movementManager.Movement != Movement.None
            ...
            switch (_brushMode)
            {
                case BrushMode.Paint:
                    ...
                    artefact.ShaderUpdater.AddVertices(brushedVertices, _activeLabel.id);
                    break;
                case BrushMode.Eraser:
                    _activeLabel.vertices.RemoveAll(v => brushedVertices.Contains(v));
                    artefact.ShaderUpdater.RemoveVertices(brushedVertices, _activeLabel.id);
                    break;
                default:
